...
puts "Length of latest tweet."
twitter_data.each do |person|
  person.each do |name, info|
    puts "#{ name }: #{ info["latest tweet"].length }"
  end
end

# 4. Each username followed by the total character count used in that user's last twenty tweets.
puts "Total character count for the previous twenty tweets."
twitter_data.each do |person|
  person.each do |name, info|
    user_name = name
    info.each do |title, tweets|
      if title == "last twenty tweets"
          tweet_char_count = 0
          tweets.each do |characters|
            tweet_char_count += characters.length
          end
          puts "#{ user_name }: #{ tweet_char_count }"
      end
    end
  end
end

# 5.a Which user has the most followers?
puts "The person with the most followers is:"
follower_total = 0
user_most_followers = nil

twitter_data.each do |person|
  person.each_pair do |name, info|
   if info["number of followers"] > follower_total
     follower_total = info["number of followers"]
     user_most_followers = name
  end
 end
end
puts "#{ user_most_followers }: #{ follower_total }"

# 5.b Which user has the most friends?
puts "The person with the most friends is:"
friends_total = 0
user_most_friends = nil

twitter_data.each do |person|
  person.each_pair do |name, info|
   if info["number of friends"] > friends_total
     friends_total = info["number of friends"]
     user_most_friends = name
  end
 end
end
puts "#{ user_most_friends }: #{ friends_total }"
#
# # 5.c Which user has the greatest number of tweets?
puts "The person with the greatest number of tweets is:"
most_tweets = 0
most_tweets_user = nil

twitter_data.each do |person|
  person.each_pair do |name, info|
   if info["number of tweets"] > most_tweets
     most_tweets = info["number of tweets"]
     most_tweets_user = name
  end
 end
end
puts "#{ most_tweets_user }: #{ most_tweets }"

# 5.d Which users have a `description` listed in `twitter_data`?
puts "The following users have a description."
twitter_data.each do |person|
  person.each do |name, info|
    if info["description"] == nil
    else
...
# 5.e Which users have a `location` listed in `twitter_data`?
puts "These users have a listed location."
twitter_data.each do |person|
  person.each do |name, info|
